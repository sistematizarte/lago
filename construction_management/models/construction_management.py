...
class material_details(models.Model):
    ...
    def _cost_total(self):
        for rec in self:
            if rec.product_qty:
                rec.cost_total = rec.cost_product_id * rec.product_qty
            else:
                rec.cost_total = 0

    product_id = fields.Many2one('product.product', 'Product')
    cost_product_id = fields.Float('Cost Product', related="product_id.standard_price", store=True)
...
class project_task(models.Model):
    ...
    equipos_ids = fields.One2many('maintenance.equipment', 'preventive_default_task_id')
